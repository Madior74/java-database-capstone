    ...
    public ResponseEntity<Map<String, String>> validateAdmin(Admin receivedAdmin) {
        ...
        try {
            if (receivedAdmin == null) {
                map.put("error", "Admin data is null");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(map);
            }
            
            String receivedUsername = receivedAdmin.getUsername();
            String receivedPassword = receivedAdmin.getPassword();
            
            System.out.println("DEBUG: Received admin username: " + receivedUsername);
            System.out.println("DEBUG: Received admin password: " + (receivedPassword != null ? "***" : "null"));
            
            if (receivedUsername == null || receivedUsername.isEmpty()) {
                map.put("error", "Username is required");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(map);
            }
            
            Admin admin = adminRepository.findFirstByUsername(receivedUsername);
            System.out.println("DEBUG: Found admin in DB: " + (admin != null));
            
            if (admin != null) {
                System.out.println("DEBUG: DB admin username: " + admin.getUsername());
                System.out.println("DEBUG: DB admin password: " + (admin.getPassword() != null ? "***" : "null"));
                
                if (admin.getPassword() != null && admin.getPassword().equals(receivedPassword)) {
            ...
            System.out.println("Error in validateAdmin: " + e.getMessage());
            e.printStackTrace();
